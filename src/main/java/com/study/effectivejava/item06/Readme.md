...
1. 박싱 : Wrapper class 를 사용하여 primitive 타입을 reference 타입으로 변환하는 과정
2. 언박싱 : reference 타입을 primitive 타입으로 변환하는 과정 


```
primitive 타입 : Stack Memory 에 실제 값을 저장
reference 타입 : Stack Memory 에 주소 값을 저장하고, 객체는 Heap 영역에 저장
```

따라서, 박싱 타입의 reference 타입 객체는 오토박싱이 이루어질 때마다 새로운 객체를 생성하므로, primitive 타입을 사용하는 것이 성능상 유리하다.
reference type 이 더 많은 메모리를 차지한다.

**일반적으로는 primitive type을 사용하되, 객체에서처럼 null을 허용하거나 generic타입에서 사용되어야 할 때, reference type을 사용하자.**
